...
class PGMFileIO {
  ...
  void readPGM() throws FileNotFoundException, IOException {
    ...
    if (!matchKey(MAGIC_PGM)) throw new IOException(filename + " : wrong magic number");
    skipComment('#');
    dimX = getInt();
    dimY = getInt();
    skipLine();
  ...
  private int getInt() throws IOException {
    String s = "";
    while ((c != '\n') && Character.isSpaceChar(c)) getChar();
  ...
  private short[] loadData(int size) throws IOException {
    ...
    for (int i = 0; i < size; i++) res[i] = (short) (data[i] & 0xFF); // unsighed !
    return res;
  }
