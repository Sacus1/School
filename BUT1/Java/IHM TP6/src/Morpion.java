...
public class Morpion extends JFrame {
  ...
  /**
   * tableau: pour verifier la fin du jeu, tableau[j]= 1 ou 2 (1 = joueur1 ; 2 = joueur2) tab: pour
   * desactiver les buttons deja selectionner (false: activer, true: desactiver)
   */
  ...
  static {
    try {
      ...
      Logger.getLogger(Morpion.class.getName())
          .log(Level.WARNING, "Unable to set Nimbus Look and Feel", e);
    }
  }
          ...
          arg0 -> {
            ...
            if (tab[boutonId]) {
              ...
              JOptionPane.showMessageDialog(
                  null, "Chose another case", "Erreur", JOptionPane.ERROR_MESSAGE);
              return;
            }
            tab[boutonId] = true;
  ...
  /** Une méthode qui renvoie a showMessageDialog pour explique le principe de jeu Help menu */
  ...
  /**
   * Une méthode pour verifier la fin du jeu et qui renvoie un boolean Vous pouvez proposer votre
   * propre solution, la méthode n'est qu'une proposition!
   */
  ...
  private void gameOver(String winner) {
    // Update scoreboard
    if (winner.equals("X")) joueurXwins += 1;
    else joueurOwins += 1;
    // calculate who's winning right now
    if (joueurOwins > joueurXwins) leader = "O";
    else leader = "X";
    Leader.setText("Leader : " + leader);
    Score.setText("Score : " + joueurXwins + ":" + joueurOwins);
    JOptionPane.showMessageDialog(
        null, "Player " + leader + " won", "Win", JOptionPane.INFORMATION_MESSAGE);
    int Oui =
        JOptionPane.showConfirmDialog(
            null, "Replay", "Morpion", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
    if (Oui == 0) nouveauJeu();
  }
