...
public class Point {
  // On enregistre les coordoné du point dans un tableau de double. Ce qui nous permettra d'ajuster
  // rapidement le code en cas de 3ème axes
  ...
  */
  public double distance(Point p) {
    return Math.sqrt(
        (p.x() - coords[0]) * (p.x() - coords[0]) + (p.y() - coords[1]) * (p.y() - coords[1]));
  }
  ...
  public Point symetrique(Point p) {
    return new Point(
        Math.sqrt((p.x() - coords[0]) * (p.x() - coords[0])) * 2 + coords[0],
