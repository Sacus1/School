...
public class Segment {
  ...
  public Point milieu() {
    return new Point(
        (extremites[0].x() + extremites[1].x()) / 2, (extremites[0].y() + extremites[1].y()) / 2);
  }
  ...
  public boolean coupe(Segment s) {
    Droite d1 =
        new Droite(extremites[0].x(), extremites[0].y(), extremites[1].x(), extremites[1].y());
    Point inter =
        new Point(
            d1.intersection(
                new Droite(
    ...
    if (this.longueur() < extremites[0].distance(inter)) return false;
    if (s.longueur() < extremites[0].distance(inter)) return false;
    return true;
  }
