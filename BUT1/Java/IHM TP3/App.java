...
class FrameSquelette extends JFrame implements MouseInputListener {
    ...
    public Pan() {
      ...
      for (int i = 0; i < im.length; i++) {
        try {
          im[i] =
              (ImageIO.read(
                  new File("/home/sacus/Cours/Java/IHM TP3/Image/im" + (i + 1) + ".png")));
        } catch (IOException e) {
          System.out.println(e.getMessage());
        }
      }
    ...
    public void paintComponent(Graphics g) {
      ...
      for (int j = 0; j < 5; j++) {
        for (int i = 0; i < 4; i++) {
          ...
          else g.drawImage(fond, j * 120 + 5, i * 120 + 5, null);
        }
      }
    }
  }
  ...
  public void mouseClicked(MouseEvent e) {
    ...
    int ancienPosX = posX;
    int ancienPosY = posY;
    if (posX >= 0 && posY > -1) img1 = im[order[posY * 5 + posX] - 1];
    posX = e.getX() / 120;
    posY = e.getY() / 120;
    if (posX >= 0 && posY > -1 && trouve[posY * 5 + posX]) return;
