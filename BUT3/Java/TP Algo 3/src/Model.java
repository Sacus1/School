...
public class Model extends Observable {
  ...
  private void addAdjacent(int i, int j, ArrayList<Vertex> closed, List<Vertex> neighbors) {
    if (i >= 0
        ...
        && j < grid[0].length
        && grid[i][j].isWalkable()
        && (closed == null || !closed.contains(grid[i][j]))) {
      neighbors.add(grid[i][j]);
      if (grid[i][j].type != Type.END
          && grid[i][j].type != Type.DEPART
          && grid[i][j].type != Type.WALL
          && grid[i][j].type != Type.EXPLORED) {
  ...
  public void findPath() {
    ...
    for (int j = 0; j < Model.HEIGHT; j++) {
      for (int i = 0; i < Model.WIDTH; i++) {
        if (grid[i][j].type == Type.AVAILABLE
            || grid[i][j].type == Type.PATH
            || grid[i][j].type == Type.EXPLORED) {
  ...
  private void run() {
    try {
      ...
      Vertex current = end;
      while (current != start) {
        if (current.type != Type.END && current.type != Type.DEPART) current.type = Type.PATH;
