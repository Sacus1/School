...
public class TestDoublures {
	...
	void test_UnPremierStub() {
		...
		when(user.getLogin()).thenReturn("Nestor"); // Définition du comportement de la doublure sur la méthode getLogin()
		System.out.println(user.getLogin());
		verify(user).getLogin(); // Vérification que la méthode getLogin() a bien été appelée
		System.out.println(user.getLogin());
		verify(user, times(2)).getLogin(); // Vérification que la méthode getLogin() a bien été appelée une fois
	}
	@Test
