...
public class Observer {
  ...
  /** Observer motion control: up/down move disabled. */
  public static final int NO_UP_DOWN_MOVE = 15;
  ...
  /**
   * Resets the projection matrix according to new viewport and projection angle values.
   *
  ...
  public void setProjection(GL2 gl) {
    ...
    gl.glLoadIdentity();
    if (perspectiveOn) glu.gluPerspective(perspectiveAngle, (double) vpWidth / vpHeight, 0.1, 20.0);
    else
      gl.glOrtho(
  ...
  public void apply(GL2 gl, float[] view) {
    ...
    if (view != null) {
      for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++) transView[i * 4 + j] = view[j * 4 + i];
      ...
      if (frontMobility != 0) {
        float cb = TRANSLATION_INC * frontMobility * (float) Math.cos(heightAngle * Math.PI / 180);
        center[0] -= cb * (float) Math.cos(azimuthAngle * Math.PI / 180);
        center[1] -= cb * (float) Math.sin(azimuthAngle * Math.PI / 180);
        center[2] +=
            TRANSLATION_INC * frontMobility * (float) Math.sin(heightAngle * Math.PI / 180);
        moved = true;
      }
      ...
      if (rightMobility != 0) {
        center[0] -=
            TRANSLATION_INC * rightMobility * (float) Math.sin(azimuthAngle * Math.PI / 180);
        center[1] +=
            TRANSLATION_INC * rightMobility * (float) Math.cos(azimuthAngle * Math.PI / 180);
        moved = true;
      }

      // Process up/down motions
      if (upMobility != 0) {
        float sa = TRANSLATION_INC * upMobility * (float) Math.sin(heightAngle * Math.PI / 180);
        center[0] += sa * (float) Math.cos(azimuthAngle * Math.PI / 180);
        center[1] += sa * (float) Math.sin(azimuthAngle * Math.PI / 180);
        center[2] += TRANSLATION_INC * upMobility * (float) Math.cos(heightAngle * Math.PI / 180);
        moved = true;
      }
  ...
  public void zoomIn() {
    if (perspectiveOn) {
      ...
      if (perspectiveAngle < PERSPECTIVE_MIN) perspectiveAngle = PERSPECTIVE_MIN;
    } else // orthographic
    {
  ...
  public void zoomOut() {
    if (perspectiveOn) {
      perspectiveAngle += PERSPECTIVE_INC;
      if (perspectiveAngle > PERSPECTIVE_MAX) perspectiveAngle = PERSPECTIVE_MAX;
    } else {
      sceneSize /= SCENE_SIZE_INC;
   ...
   *
   * @param control motion type: - TURN_RIGHT, TURN_LEFT, NO_RIGHT_LEFT_TURN, - TURN_UP, TURN_DOWN,
   *     NO_UP_DOWN_TURN, - MOVE_FRONT, MOVE_BACK, NO_FRONT_BACK_MOVE, - MOVE_RIGHT, MOVE_LEFT,
   *     NO_RIGHT_LEFT_MOVE, - MOVE_UP, MOVE_DOWN, NO_UP_DOWN_MOVE
   */
   ...
   *
   * @param volume Default volume to display (float array containing xmin, xmax, ymin, ymax, zmin
   *     the zmax)
   */
   ...
   *
   * @param volume Accessible volume bounds. (float array containing xmin, xmax, ymin, ymax, zmin
   *     the zmax)
   */
