...
public class Matrix
{
  /** Representation interne : une matrice 4x4 correspondant aux axes
    * principaux et au vecteur translation */
  float[][] termes = new float[4][4];
  ...
  /** Construit une matrice a partir des composantes des 3 axes X, Y et Z
    * et des composantes d'un vecteur de translation.
  ...
  public void mult (Matrix mat)
  {
    for (int i = 0; i < 4; i++)
      for (int j = 0; j < 4; j++)
      {
        tmp[i][j] = 0.0f;
        for (int k = 0; k < 4; k++)
          tmp[i][j] += termes[i][k] * mat.termes[k][j];
      }
    // copie du resultat dans la matrice
    for (int i = 0; i < 4; i++)
      for (int j = 0; j < 4; j++) termes[i][j] = tmp[i][j];
  }
  ...
  public void multLeft (Matrix mat)
  {
    for (int j = 0; j < 4; ++j)
      for (int i = 0; i < 4; ++i)
      {
        tmp[j][i] = 0.0f;
        for (int k = 0; k < 4; ++k)
          tmp[j][i] += mat.termes[j][k] * termes[k][i];
      }
    // copie du resultat dans la matrice
    for (int j = 0; j < 4; ++j)
      for (int i = 0; i < 4; ++i) termes[j][i] = tmp[j][i];
  }
